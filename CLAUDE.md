...
- `api/_lib/` — moduli condivisi (non sono endpoint):
  - `email-shell.js` — guscio email **unico** e **unico aspetto**: header **nero** col logo chiaro, corpo crema, footer legale. Stili INLINE email-safe. Il logo viaggia come allegato inline `cid:logo-dark.png` (da `logo-dark-data.js` in base64) così è sempre visibile anche nei client che bloccano le immagini remote (Libero, Outlook). Se cambia palette/logo/dati legali → si cambia **qui**, e vale per tutte le mail. ⚠️ Fino al 31/08/2026 non era vero: esistevano **quattro** gusci (questo, allora crema; `darkShell` duplicato in `send_lead.mjs` **e** in `send_drip.mjs`; `lightShell` in `send_lead.mjs`), e i due script riscrivevano a mano anche il footer coi dati legali — P.IVA in tre posti. Ora entrambi importano da qui e il flag `--light` di `send_lead.mjs` è accettato ma ignorato.
